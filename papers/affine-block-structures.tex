...
where:
\begin{itemize}
    \item $\nu \ge 0$ is the 2-adic valuation $v_2(x)$ (leading even steps in E*(OE)+E+ pattern)
    \item $\alpha \ge 1$ is the 2-adic valuation $v_2(x/2^\nu + 1)$ of the odd core plus one
    \item $\rho \ge 1$ is an odd integer parameter
    \item $\kappa \ge \alpha$ is the block length (number of even steps in the block)
...
The block parameters $(\alpha, \rho, \kappa)$ describe the structure of the odd core. We first define the affine functions for odd values ($\nu = 0$), then show how $\nu$ prefixes additional even steps for even starting values.
...
For even $x$ with $\nu = v_2(x) > 0$, there are $\nu$ leading even steps (E* prefix) before reaching the odd core. These initial divisions by 2 are represented by the $2^\nu$ factor:
...
since the $\nu$ leading even steps have already been consumed in reaching the odd core, and the successor starts after the block operations on that odd core.
