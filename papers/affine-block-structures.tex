...
\begin{abstract}
We present a simplified framework for analyzing Collatz sequences through affine block structures. Each block is characterized by four parameters $(\alpha, \nu, \rho, \kappa)$ and a scaling parameter $t$ that enumerates instances of the block. The block defines two affine functions: $x(B,t)$ mapping $t$ to integers within the block, and $\mathrm{succ}_B(B,t)$ giving the starting value of the successor block. This approach focuses exclusively on lattice-wide affine relationships without attempting to model internal block dynamics.
\end{abstract}
...
The function $\mathrm{succ}_B(B,t)$ gives the starting value of the successor block:
\[
\mathrm{succ}_B(B,t) = \frac{3^\alpha \cdot (\rho + t \cdot 2^{\kappa - \alpha}) - 1}{2^{\kappa - \alpha}}
\]

Expanding:
\[
\mathrm{succ}_B(B,t) = \frac{3^\alpha \rho - 1}{2^{\kappa - \alpha}} + 3^\alpha \cdot t
\]

This is an affine function $\mathrm{succ}_B(t) = m_{\mathrm{succ}} \cdot t + c_{\mathrm{succ}}$ with:
\begin{align*}
...
\mathrm{succ}_B(t) &= \frac{3^2 \cdot 9 - 1}{2^4} + 3^2 \cdot t = \frac{80}{16} + 9t = 5 + 9t
\end{align*}

For $t = 0$: $x(0) = 35$ and $\mathrm{succ}_B(0) = 5$.
